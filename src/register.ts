 ...
 * - `/ct-init`, `/ct-add`, … individual alias commands
 * - `ct_add`, `ct_remove`, `ct_toggle`, `ct_status` LLM tools
 */
export function registerCatalog(pi: ExtensionAPI): void {
  // ----- /ct main command --------------------------------------------------
  pi.registerCommand("ct", {
    description: "Catalog management: add, remove, toggle, init, status, and more",
  ...
  pi.registerTool({
    ...
    label: "Catalog Reset",
    description:
      "Full nuke: uninstall all @pi-stef packages and delete config files.",
