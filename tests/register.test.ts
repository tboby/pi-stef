...
describe("registerCatalog", () => {
  ...
  it("registers /ct subcommand aliases (ct-init, ct-add, etc.)", () => {
    const { pi, commands } = mockPi();
    registerCatalog(pi);
  ...
  it("registers LLM tools (ct_add, ct_remove, ct_update, ct_toggle, ct_status, ct_reset)", () => {
  ...
  it("calls registerCommand the expected number of times", () => {
    ...
    // 1 main (/ct) + N subcommand aliases
