...
const { runVerification } = await import("../src/tools/followup");
...
function makeFixture(opts: {
  ...
  const root = mkdtempSync(path.join(tmpdir(), "followup-verify-gate-"));
...
describe("sf_team_followup runVerification: existing-behavior preservation", () => {
  ...
  it("throws on non-zero exit with cwd, exit status, and stderr/stdout snippet", () => {
    const { root, dispose } = makeFixture({
      packageJson: { packageManager: "npm@10.2.0", scripts: { typecheck: "true", test: "true" } },
    });
    try {
