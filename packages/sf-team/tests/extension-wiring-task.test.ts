...
describe("extension wiring: fh_team_plan / fh_team_task surface as `<base>` + `<base>_resume`", () => {
  ...
  it("fh_team_plan and fh_team_task expose tool-specific knobs in their schemas (not stubs)", async () => {
    const pi = new FakePi();
    fhTeamExtension(pi as never);
    const plan = pi.tools.find((t) => t.name === "fh_team_plan");
    ...
    const task = pi.tools.find((t) => t.name === "fh_team_task");
